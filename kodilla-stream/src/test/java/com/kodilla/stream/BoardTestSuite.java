...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        double average = project.getTaskLists().stream()
                .filter(ts -> ts.getName().equals("In progress"))
                .flatMap(tl -> tl.getTasks().stream())
                .filter(t -> !t.getCreated().isAfter(LocalDate.now()))  //wyklucza nierozpoczete
                .mapToDouble(task -> Period.between(task.getCreated(), LocalDate.now()).getDays())
                .average()
                .orElse(0);

        //Then
        Assert.assertEquals(10, average, 0.01); //chyba coś źle policzyliśmy wcześniej do expected ;)
    }
}
